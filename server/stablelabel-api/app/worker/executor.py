...
from app.worker.text_extraction import _extract_text_from_bytes, _MAX_FILE_SIZE, _STREAM_THRESHOLD
...
class JobExecutor:
    ...
    def _record_scan_result(
        self, job: Job, drive_id: str, item_id: str, filename: str,
        outcome: str, *, classification: str | None = None,
        confidence: float | None = None, label_applied: str | None = None,
    ) -> None:
        ...
        for i in range(0, len(remaining), _LABELLING_BATCH_SIZE):
            ...
            for file_info in batch:
                ...
                if use_policies:
                    ...
                    if file_classification and file_classification.error == "deferred":
                        ...
                        self._record_scan_result(
                            job, drive_id, item_id, filename, "skipped",
                        )
                        continue
                    ...
                    if not label_id:
                        ...
                        self._record_scan_result(
                            job, drive_id, item_id, filename, "skipped",
                            classification=top_entity, confidence=top_conf,
                        )
                ...
                if dry_run:
                    ...
                    self._record_scan_result(
                        job, drive_id, item_id, filename, "labelled",
                        classification=top_entity, confidence=top_conf,
                ...
                if result.status == JobStatus.COMPLETED:
                    ...
                    if result.error and "Unsupported" in result.error:
                        ...
                        batch_failed += 1
                        self._record_scan_result(
                            job, drive_id, item_id, filename, "failed",
                            classification=top_entity, confidence=top_conf,
                            label_applied=label_id,
                        )
                        logger.warning(
    ...
    async def _load_tenant_policies(self, customer_tenant_id: uuid.UUID) -> list[PolicyRule]:
        ...
        from app.worker.shared import load_tenant_policies
        return await load_tenant_policies(self._db, customer_tenant_id)
