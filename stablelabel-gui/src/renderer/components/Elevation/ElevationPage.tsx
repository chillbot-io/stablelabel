    ...
    <div className="flex h-full">
      ...
      <div className="w-56 flex-shrink-0 border-r border-white/[0.06] bg-zinc-950 flex flex-col">
        <div className="p-3 border-b border-white/[0.06]">
          <h2 className="text-sm font-semibold text-zinc-300">Elevation</h2>
          <p className="text-[10px] text-zinc-500 mt-0.5">Just-in-time privilege management</p>
        </div>
        <div className="flex-1 py-2">
          {sections.map((s) => (
            <button
              ...
              className={`w-full text-left px-3 py-2.5 transition-colors ${active === s.id ? 'bg-white/[0.06] border-l-2 border-yellow-400' : 'hover:bg-white/[0.04] border-l-2 border-transparent'}`}
            >
