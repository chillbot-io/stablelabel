...
export default function LabelDlpAlignment() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-semibold text-zinc-300 mb-1">Label-DLP Alignment</h3>
        <p className="text-xs text-zinc-500">Check which sensitivity labels are backed by DLP rules and which have gaps.</p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white/[0.03] rounded-lg p-3 text-center">
              <dt className="text-[10px] text-zinc-500">Checked</dt>
              <dd className="text-lg font-bold text-zinc-200">{result.LabelsChecked}</dd>
            </div>
            <div className="bg-emerald-400/5 border border-green-500/20 rounded-lg p-3 text-center">
              <dt className="text-[10px] text-zinc-500">Aligned</dt>
              <dd className="text-lg font-bold text-emerald-400">{result.AlignedLabels?.length ?? 0}</dd>
            </div>
            <div className={`rounded p-3 text-center ${unprotectedCount > 0 ? 'bg-yellow-500/5 border border-yellow-500/20' : 'bg-white/[0.03]'}`}>
              <dt className="text-[10px] text-zinc-500">Unprotected</dt>
              <dd className={`text-lg font-bold ${unprotectedCount > 0 ? 'text-yellow-400' : 'text-zinc-400'}`}>{unprotectedCount}</dd>
            </div>
          </div>
          ...
          {result.AlignedLabels && result.AlignedLabels.length > 0 && (
            <div className="bg-white/[0.03] rounded-xl p-4">
              <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Aligned Labels</h4>
              <div className="space-y-1">
                {result.AlignedLabels.map((a, i) => (
                  <div key={i} className="flex items-center justify-between px-2.5 py-1.5 bg-white/[0.06] rounded-lg text-xs">
                    <span className="text-zinc-200">{a.LabelName}</span>
                    <span className="text-zinc-500">{a.DlpRule}</span>
                  </div>
                ))}
              </div>
            </div>
