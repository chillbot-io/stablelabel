...
export default function ProtectionLogs() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-sm font-semibold text-zinc-300 mb-1">Protection Logs</h3>
        <p className="text-xs text-zinc-500">AIP protection tracking logs — view who accessed protected content.</p>
      </div>
      ...
      {entries && (
        <div className="bg-white/[0.03] rounded-xl p-4 space-y-3">
          <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">{entries.length} Log {entries.length === 1 ? 'Entry' : 'Entries'}</h4>
          {entries.length === 0 ? (
            <p className="text-xs text-zinc-500">No log entries match your search criteria.</p>
          ) : (
    ...
    <div className="p-3 bg-white/[0.06] rounded-lg">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          {action && <span className="text-[10px] px-1.5 py-0.5 bg-blue-500/10 text-blue-400 rounded-lg">{String(action)}</span>}
          <span className="text-sm text-zinc-200">{contentName ? String(contentName) : 'Unknown'}</span>
        </div>
      ...
      {expanded && <pre className="mt-2 p-2 bg-white/[0.03] rounded-lg text-xs text-zinc-400 overflow-auto max-h-32">{JSON.stringify(entry, null, 2)}</pre>}
    </div>
  );
}
...
function RawJson({ data }: { data: unknown }) {
  ...
  return (
    <div className="border-t border-white/[0.06] pt-3">
      <button onClick={() => setO(!o)} className="text-xs text-zinc-500 hover:text-zinc-300">{o ? '▾ Hide' : '▸ Show'} all raw JSON</button>
