...
export default function OnboardingPolicy() {
  ...
  if (loading) return <div className="space-y-3">{[1,2].map(i => <div key={i} className="h-16 bg-white/[0.06] rounded-lg animate-pulse" />)}</div>;
  if (error) return <div className="p-3 bg-red-900/20 border border-red-800 rounded-lg text-sm text-red-300">{error}</div>;

  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-semibold text-zinc-300 mb-1">Onboarding Control Policy</h3>
        <p className="text-xs text-zinc-500">Control which users can use Azure Information Protection in your tenant.</p>
      </div>
      ...
      {saveMsg && <div className={`p-2 rounded-lg text-xs ${saveMsg.type === 'error' ? 'bg-red-900/20 border border-red-800 text-red-300' : 'bg-green-900/20 border border-green-800 text-green-300'}`}>{saveMsg.text}</div>}

      <button onClick={handleSave} disabled={saving} className="px-4 py-2 text-xs font-medium text-white bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 rounded-lg transition-colors">
