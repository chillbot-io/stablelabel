...
const actionBadge = (action: string | null) => {
  switch (action) {
    ...
    default: return { text: action ?? 'None', color: 'bg-white/[0.08] text-zinc-400' };
  }
};
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-2 border-b border-white/[0.06]">
        <input type="text" placeholder="Search retention labels..." value={search} onChange={e => setSearch(e.target.value)} className="w-full px-2.5 py-1.5 text-xs bg-white/[0.06] border border-white/[0.08] rounded-lg text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-amber-500" />
      </div>
      <div className="px-3 py-1.5 text-xs text-zinc-500 border-b border-white/[0.04]">{labels.length} retention {labels.length === 1 ? 'label' : 'labels'}</div>
      <div className="flex-1 overflow-y-auto py-1">
        {paginated.length === 0 ? <p className="p-4 text-xs text-zinc-600">No retention labels found.</p> : <>{paginated.map(label => {
          const badge = actionBadge(label.RetentionAction);
          return (
            <button key={label.Guid ?? label.Name} onClick={() => onOpenLabel(label.Name)} className="w-full text-left px-3 py-2 hover:bg-white/[0.06] transition-colors group">
              <div className="flex items-center justify-between">
                <span className="text-sm text-zinc-200 group-hover:text-white truncate">{label.Name}</span>
                <span className={`text-[10px] px-1.5 py-0.5 rounded-lg ${badge.color}`}>{badge.text}</span>
              </div>
              <div className="flex items-center gap-2 mt-0.5">
                {label.RetentionDuration != null && <span className="text-[10px] text-zinc-500">{label.RetentionDuration} days</span>}
      ...
      <div className="p-2 border-t border-white/[0.06] space-y-1.5">
        <button onClick={onNewLabel} className="w-full py-1.5 text-xs text-amber-300 hover:text-amber-200 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/20 rounded-lg transition-colors">+ New Retention Label</button>
