...
export default function SnapshotDetail({ snapshotName, onDeleted, onCompare }: Props) {
  ...
  if (error || !snap) return <div className="p-6"><div className="bg-red-900/20 border border-red-800 rounded p-4 text-sm text-red-300">{error ?? 'Not found'}</div></div>;
    ...
    <div className="p-6 space-y-6 max-w-3xl">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Captured {snap.CreatedAt} by {snap.CreatedBy}</p>
        </div>
        <span className="px-2 py-1 text-xs bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded">{snap.Scope}</span>
      </div>
      ...
      {snap.Items && Object.keys(snap.Items).length > 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Captured Items</h3>
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(snap.Items).map(([category, count]) => (
              <div key={category} className="flex items-center justify-between px-2.5 py-1.5 bg-gray-800 rounded">
      ...
      <div className="flex items-center gap-3">
        <button onClick={handleCompareLive} disabled={comparing} className="px-4 py-2 text-xs font-medium text-blue-300 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/20 rounded transition-colors disabled:opacity-50">
        ...
        <button onClick={() => setShowDelete(true)} className="px-4 py-2 text-xs font-medium text-red-300 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 rounded transition-colors">
          Delete
        </button>
      </div>

      {compareError && <div className="p-3 bg-red-900/20 border border-red-800 rounded text-sm text-red-300">{compareError}</div>}
...
function Card({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="bg-gray-900 border border-gray-800 rounded p-3">
      ...
      <dd className={`text-sm text-gray-200 truncate ${mono ? 'font-mono text-xs' : ''}`} title={value}>{value}</dd>
    </div>
  );
}
